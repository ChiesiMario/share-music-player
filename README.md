...
### 3. 一鍵 Docker 容器化部署 (推薦 🌟)
本專案已完整支援 Docker 與 `docker-compose`，非常適合在您的個人伺服器上快速部署與維護：

1. **環境配置**：
   修改 `docker-compose.yaml` 中的環境變數 `UPLOAD_PASSWORD`。
2. **啟動容器**：
   在專案目錄下執行：
   ```bash
   ...
   預設會在 `29099` 埠號啟動服務。
3. **資料持久化**：
   Docker 配置已掛載 `.cache` 與 `uploads` 目錄為 Volume，即使重啟容器也不會遺失已上傳的音樂檔案。
4. **進階維護腳本**：
   專案 `scripts/` 目錄下包含了針對特殊網路環境（如中國大陸鏡像站）修復與自動部署的實用腳本。
